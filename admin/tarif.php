                ...
                <tbody>
                    ...
                    <?php foreach ($data as $row) : ?>
                        <tr>
                            ...
                            <td><?= $row["tipe"] ?></td>
                            <td><?= $row["Nominal"] ?></td>
                            <td><a class="icon-hps" onclick="return confirm('Yakin ingin menghapus tarif angkatan : <?= $row['Angkatan'] ?>')" href="../config/action/delete/all.php?id=<?= $row['id'] ?>&tabel=tarif&primary=id">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="curentcolor" viewBox="0 0 24 24">
