                    ...
                    <?php foreach ($data as $row) : ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $id_cek = $row['Nis'];
                                $query = mysqli_query($konek, "SELECT siswa.Nis, pembayaran.Nis FROM siswa INNER JOIN pembayaran on siswa.Nis=pembayaran.Nis WHERE siswa.Nis='$id_cek'");
                                $cek = mysqli_num_rows($query);
                                if ($cek == 0) {
                                ?>
                                    <a onclick="return confirm('Yakin ingin menghapus siswa : <?= $row['Nama_Siswa'] ?>')" class="icon-hps" href="../config/action/delete/all.php?id=<?= $row['Nis']; ?>&tabel=siswa&primary=Nis">
