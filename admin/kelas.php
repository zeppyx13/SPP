                    ...
                    <?php foreach ($data as $row) : ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $id_cek = $row['idkelas'];
                                $query = mysqli_query($konek, "SELECT kelas.idkelas, siswa.idkelas FROM kelas INNER JOIN siswa on kelas.idkelas=siswa.idkelas WHERE kelas.idkelas='$id_cek'");
                                $cek = mysqli_num_rows($query);
                                if ($cek == 0) {
                                ?>
                                    <a onclick="return confirm('Yakin ingin menghapus kelas : <?= $row['kelas']; ?> <?= $row['jurusan']; ?>')" class="icon-hps" href="../config/action/delete/all.php?id=<?= $row['idkelas'] ?>&tabel=kelas&primary=idkelas">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="curentcolor" viewBox="0 0 24 24">
                                ...
                                <?php } else { ?>
                                    <svg class="icon-hps" xmlns="http://www.w3.org/2000/svg" fill="curentcolor" viewBox="0 0 24 24">
                                        <path d="M7 10h10v4H7z" />
                                        <path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z" />
                                    </svg>
                                <?php } ?>
                                <a class="icon-edit" href="../config/action/update/kelas.php?id=<?= $row['idkelas']; ?>">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="curentcolor" viewBox="0 0 24 24">
